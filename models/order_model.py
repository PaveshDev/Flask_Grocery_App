"""Order Model - Enhanced with shopping cart, notifications, and order tracking
"""
...
def place_order(user_id, delivery_address, delivery_phone, payment_method='cash'):
    ...
    for product_id, quantity, price, stock, *rest in cart_items:
        # Get product name
    ...
    # Create notification
    create_notification(cursor, user_id, 'order_placed', 
...
def update_order_status(order_id, new_status):
    ...
    if new_status in notification_messages:
        ...
        create_notification(cursor, user_id, f'order_{new_status}', title, message, order_id)
...
def create_notification(cursor, user_id, notif_type, title, message, order_id=None):
...
def get_order_details(order_id):
    """Get detailed items in an order"""
    db = connect_db()
    cursor = db.cursor()
    ...
    # Get order items
    cursor.execute("""
        SELECT oi.order_item_id, oi.product_name, oi.quantity, oi.unit_price, oi.subtotal
        FROM order_items oi
        WHERE oi.order_id = %s
    """, (order_id,))
    items = cursor.fetchall()
